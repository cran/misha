...
public:
    ...
    // Constructor that accepts a shared GenomeSeqFetch pointer for caching across multiple scorers
    GenomeSeqScorer(GenomeSeqFetch* shared_seqfetch, bool extend = true, char strand = 0);
...
protected:
    ...
    GenomeSeqFetch m_seqfetch_owned;  // Owned instance (when not using shared)
    GenomeSeqFetch* m_seqfetch_ptr;   // Pointer to fetch object (either owned or shared)
};
